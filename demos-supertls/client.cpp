...
#define CERTF  "server_rsa.crt"
#define KEYF   "server_rsa.key"
...
int main ()
{
  ...
  /*
  char *buffer;
  buffer = (char *)malloc((1048576+1)*sizeof(char)); // Enough memory for file + \0
  */
  ...
  /*
  int filelen = 1073741824;
  int max_size = 16250;
  err = 0;
  int total_size = max_size;
  
  while(total_size < filelen){
    err = SSL_read (ssl, buffer+err, max_size);  CHK_SSL(err);
    total_size += err;
  }
